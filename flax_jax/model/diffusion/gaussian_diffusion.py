...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, rng, noise=None):
        """
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            rng, subkey = jax.random.split(rng)
            img = jax.random.normal(subkey, shape)
    ...
    def _prior_bpd(self, x_start):
        ...
        t = jnp.full((batch_size,), self.num_timesteps - 1, dtype=jnp.int32)
    ...
    def calc_bpd_loop(self, model, x_start, rng, clip_denoised=True, model_kwargs=None):
        """
        ...
        for t in reversed(range(self.num_timesteps)):
            ...
            noise = jax.random.normal(step_rng, shape=x_start.shape)
        ...
        total_bpd = jnp.sum(vb, axis=1) + prior_bpd
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    :param timesteps: a jnp array of indices into the array to extract.
    :param broadcast_shape: a larger shape of K dimensions with the batch
                            dimension equal to the length of timesteps.
    :return: a tensor of shape [batch_size, 1, ...] where the shape has K dims.
    """
    res = jnp.array(arr)[timesteps].astype(jnp.float32)

    # Add extra dimensions to match broadcast_shape
